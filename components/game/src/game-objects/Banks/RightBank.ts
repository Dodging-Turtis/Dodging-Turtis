...
export class RightBank extends Phaser.GameObjects.Container {
  scene: AbstractScene;
  ...
  private addExtensions() {
    ...
    for (let i = 0; i < 3; ++i) {
      ...
      this.extensions[i].setOrigin(0, 1);
    }
  ...
  private addBanks() {
    ...
    const { width, height } = this.scene.grs.designDim;
    for (let i = 0; i < 3; ++i) {
      this.banks[i] = this.scene.add.image(0, 0, 'right_bank_1');
