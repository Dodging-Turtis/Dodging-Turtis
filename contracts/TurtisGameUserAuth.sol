...
contract TurtisGameUserAuth {
  ...
  function createAccount(string memory _nickName) public {
    require(
      bytes(_nickName).length <= 32,
      "Nickname should not be more than 32 characters"
    );
    require(
      bytes(_nickName).length > 2,
      "Nick name should be atleast 3 characters"
    );
  ...
  function addressByNickName(string memory _nickName)
    public
    view
    returns (address)
  {
  ...
  function nickNameByAddress(address _address)
    public
    view
